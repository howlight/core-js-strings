...
function removeFirstOccurrences(str, value) {
  ...
  const substring = str.substr(startIndex, valueLength);
  const result = str.replace(substring, '');
  return result;
}
...
function removeLastOccurrences(str, value) {
  ...
  const endIndex = startIndex + valueLength;
  const string1 = str.slice(0, startIndex);
  const string2 = str.slice(endIndex);
...
function countVowels(str) {
  const matchesArray = str.match(/[aeiouy]/gi);
...
function isPalindrome(str) {
  /* удалить специальные символы, пробелы и сделать строчными */
...
function findLongestWord(sentence) {
  ...
  // Вернуть первый отсортированный элемент массива
...
function encodeToRot13(str) {
  ...
  // Строка с зашифрованным алфавитом. rotate => 13
  const cipherAlphabet = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  // Замена букв в строке на основе индексов
  return str.replace(
    /[a-z]/gi,
    (letter) => cipherAlphabet[alphabet.indexOf(letter)]
  );
}
